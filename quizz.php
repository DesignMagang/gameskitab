...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'add_quiz') {
    if (isset($_POST['quiz_name']) && !empty($_POST['quiz_name'])) {
        ...
        // Changed table name from 'quiz' to 'quizz'
        $stmt = $conn->prepare("INSERT INTO quizz (user_id, name) VALUES (?, ?)");
        $stmt->bind_param("is", $user_id, $quiz_name);

        if ($stmt->execute()) {
            // Redirect dengan notifikasi sukses (Post/Redirect/Get pattern)
            header("Location: quizz.php?success=added"); // Changed filename
            exit();
        } else {
            // Redirect dengan notifikasi error
            header("Location: quizz.php?error=add_failed&msg=" . urlencode($stmt->error)); // Changed filename
            exit();
        }
        $stmt->close();
    } else {
        // Redirect jika nama kuis kosong
        header("Location: quizz.php?error=add_failed&msg=" . urlencode("Nama kuis tidak boleh kosong.")); // Changed filename
        exit();
    }
}
...
// Mengambil data kuis dari tabel 'quizz'
$stmt = $conn->prepare("SELECT * FROM quizz WHERE user_id = ? ORDER BY name ASC"); // Changed table name
...
<head>
    <meta charset="UTF-8">
    <title>Quizz Dashboard</title> <script src="https://cdn.tailwindcss.com"></script>
        ...
        <div class="p-6 rounded-lg w-full"> 
            ...
            <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 backdrop-blur-sm">
                <div class="bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-lg shadow-xl w-full max-w-sm border border-gray-700 transform transition-all duration-300 animate-fade-in">
                    ...
                    <form action="quizz.php" method="POST"> <input type="hidden" name="action" value="add_quiz"> 
                        <input type="text" name="quiz_name" placeholder="Nama Kuis" required 
            ...
            <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 backdrop-blur-sm">
                <div class="bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-lg shadow-xl w-full max-w-sm border border-gray-700 transform transition-all duration-300 animate-fade-in">
                    ...
                    <form action="edit_quizz.php" method="POST"> <input type="hidden" name="quiz_id" id="editQuizId">
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <div id="quiz-card-<?= $row['id'] ?>" class="gradient-border bg-gray-800 p-4 rounded-lg shadow-lg flex flex-col justify-between glow-card hover:bg-gray-700 transition-all duration-300">
                            <div class="flex justify-between items-start">
                                <a href="quizz_question.php?id=<?= $row['id'] ?>"  onclick="hideQuizCard(<?= $row['id'] ?>);" 
