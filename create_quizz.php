...
if ($quiz_id === 0 || $question_id === 0) {
    die("ID Kuis atau ID Pertanyaan tidak ditemukan.");
}
...
// Ambil data pertanyaan yang akan diedit (menambahkan time_limit)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'update_question') {
    ...
    $time_limit = isset($_POST['time_limit']) ? intval($_POST['time_limit']) : 30; // Ambil batas waktu, default 30
    ...
    // Update data di database (menambahkan time_limit)
    ...
    if ($stmt->execute()) {
        header("Location: quizz_question.php?id=$quiz_id&success=updated");
        exit();
    } else {
        header("Location: quizz_question.php?id=$quiz_id&error=db_update_failed&msg=" . urlencode($stmt->error));
        exit();
    }
    $stmt->close();
}
...
<head>
    ...
    <title>Edit Pertanyaan untuk Quizz: <?= $quiz_name ?></title>
...
<body class="p-6 relative">
    <div class="modal-glass p-6 rounded-xl max-w-lg w-full animate__animated animate__fadeIn">
        <h2 class="text-xl font-bold text-white mb-4">Edit Pertanyaan Quizz: <span class="text-amber-300"><?= $quiz_name ?></span></h2>
